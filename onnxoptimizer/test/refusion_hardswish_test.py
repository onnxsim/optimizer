...
"""Re-fusion test for the HardSwish activation.

The input graph is produced by an *actual* ``torch.onnx`` export of
``torch.nn.Hardswish``. At opset < 14 torch decomposes it into a
``HardSigmoid`` (alpha=1/6, beta=1/2) + ``Mul`` pair. The ``fuse_hardswish``
pass should re-fuse that pair back into the single ``ai.onnx`` ``HardSwish``
operator.
"""
...
class TestRefuseHardSwish(unittest.TestCase):
    TARGET_OPSET = 22

    def test_fuse_hardswish_from_torch(self):
        model = export_decomposed(
            nn.Hardswish(), torch.randn(2, 4, 8), export_opset=13
        )

        # Precondition: torch emitted the decomposed HardSigmoid + Mul pattern.
        self.assertIn("HardSigmoid", op_types(model))
        self.assertIn("Mul", op_types(model))

        optimized = onnxoptimizer.optimize(
            model, ["fuse_hardswish", "eliminate_deadend"]
        )
        relabel_opset(optimized, self.TARGET_OPSET)
        ops = op_types(optimized)

        self.assertEqual(ops.count("HardSwish"), 1)
        self.assertNotIn("HardSigmoid", ops)
