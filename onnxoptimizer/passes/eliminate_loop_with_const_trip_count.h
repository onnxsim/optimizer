...
// analogue of eliminate_if_with_const_cond: both exist to turn a control-flow
// construct that most downstream ONNX consumers either can't ingest at all,
// or ingest only in restricted form, into plain feed-forward ops once the
// control decision is already known at graph-simplification time. A `Loop`
// over a statically-known range -- the ONNX shape emitted for a Python
// `for i in range(N): ...` with no `break` -- is a common case: TVM's Relax
// ONNX frontend, for one, has no `Loop` support at all.
//
