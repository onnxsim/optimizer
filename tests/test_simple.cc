...
TEST(OptimizerTest, NopReshape) {
  ...
  auto optimized_model = onnx::optimization::Optimize(
      model, {"eliminate_nop_reshape", "eliminate_deadend"});
...
TEST(OptimizerTest, EliminateDuplicateInitializerTrustHash) {
  for (bool trust : {true, false}) {
    ...
    auto add_init =
        [&](const std::string& name, onnx::TensorProto_DataType dtype,
    ...
    auto optimized = onnx::optimization::Optimize(
        model, {"eliminate_duplicate_initializer"});
...
// HashTypedFieldBounded (cse_util.h) samples a typed-field tensor's parsed
// byte buffer via the same HashBytesBounded used by HashRawDataBounded, so
// it has the same bounded-sampling collision behavior past kFullHashLimit --
// this test constructs that worst case for a typed-field (int64_data) tensor
// pair and confirms eliminate_duplicate_initializer still keeps both
// (CSETensorCompare's exact ParseTensorData<T> comparison is the sole source
// of truth for equality, not the hash), alongside a genuinely identical
// large typed-field pair still getting deduped as before.
TEST(OptimizerTest, EliminateDuplicateInitializerLargeTypedField) {
  // Same size/offset reasoning as EliminateDuplicateInitializerLargeRawData:
  // past kFullHashLimit (4096) and past the point where kMaxSamples (256)
  // windows of kSampleWindow (64) bytes stop covering the whole buffer, so
  // offset 300 (in the gap between the first window [0, 64) and the
  // second, starting at stride = max(64, n/256) = 390) is never sampled.
  // HashTypedFieldBounded hashes ParseTensorData<int64_t>'s output bytes
  // with this exact same function, so the collision transfers directly.
  constexpr size_t n_bytes = 100000;
  static_assert(n_bytes % sizeof(int64_t) == 0, "must divide evenly");
  constexpr size_t n_elems = n_bytes / sizeof(int64_t);

  auto make_vals = [&](char fill, int flip_offset = -1, char flip_val = 0) {
    std::string bytes(n_bytes, fill);
    if (flip_offset >= 0)
      bytes[flip_offset] = flip_val;
    std::vector<int64_t> vals(n_elems);
    std::memcpy(vals.data(), bytes.data(), n_bytes);
    return std::make_pair(bytes, vals);
  };

  auto [base_bytes, base_vals] = make_vals('A');
  auto [gap_bytes, gap_vals] = make_vals('A', 300, 'B');

  ASSERT_EQ(
      onnx::optimization::HashBytesBounded(base_bytes.data(),
                                           base_bytes.size()),
      onnx::optimization::HashBytesBounded(gap_bytes.data(), gap_bytes.size()))
      << "test assumption: the two byte buffers hash-collide despite differing";
  ...
  auto add_init = [&](const std::string& name,
    ...
    auto* t = graph->add_initializer();
    t->set_name(name);
    t->set_data_type(onnx::TensorProto_DataType_INT64);
    t->add_dims(static_cast<int64_t>(n_elems));
    for (auto v : vals)
      t->add_int64_data(v);
  };
  // w1/w2: hash-colliding but element-distinct -- must NOT be merged.
  add_init("w1", base_vals);
  add_init("w2", gap_vals);
  // w3/w4: genuinely identical to each other (but distinct from base) --
  // must still be merged. A different fill byte, not just a different
  // offset, so this pair can't also collide with w1/w2's hash.
  auto [other_bytes, other_vals] = make_vals('C');
  (void)other_bytes;
  add_init("w3", other_vals);
  add_init("w4", other_vals);
